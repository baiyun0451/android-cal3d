...
bool Model::onInit(const std::string& strFilename)
{
  ...
  if(!file)
  {
    LOG(("Failed to open model configuration file '" + strFilename + "'.").c_str());
    return false;
  }
  ...
  for(line = 1; ; line++)
  {
    ...
    if(!file)
    {
      LOG(("Error while reading from the model configuration file '" + strFilename + "'.").c_str());
      return false;
    }
    ...
    if((pos == std::string::npos) || (strBuffer[pos] != '='))
    {
      LOG((strFilename + "(  line#  ): Invalid syntax.").c_str());
      return false;
    }
    ...
    {
      // load core skeleton
      LOG(("Loading skeleton '" + strData + "'...").c_str());
    ...
    {
      // load core animation
      LOG(("Loading animation '" + strData + "'...").c_str());
    ...
    {
      // load core mesh
      LOG(("Loading mesh '" + strData + "'...").c_str());
    ...
    {
      // load core material
      LOG(("Loading material '" + strData + "'..." ).c_str());
